...
#============================================================================================================================
src12="/etc/apt/sources.list"                           #-- переменная сорц листа
...
#---------------------------------------
# sharefolder="/usr/share/customconf/"
...
[ -f $hellosh ] || touch $hellosh | printf "${C_WHITE} Создаем файл: ${C_YELLOW}$hellosh${C_RESET}\n"
...
[ -f $failsrc ] || touch $failsrc | printf "${C_WHITE} Создаем файл: ${C_YELLOW}$failsrc${C_RESET}\n"
...
EOF
apt update
apt install apt-transport-https && apt install ca-certificates -y
...
do
    ...
    printf "${C_WHITE} Установлен пакет: ${C_YELLOW}$app${C_RESET}\n"
done
...
echo "net.core.default_qdisc=fq" >> $sysctlc12 
echo "net.ipv4.tcp_congestion_control=bbr" >> $sysctlc12

#ICMP ingore - optional
# echo "net.ipv4.icmp_ignore_bogus_error_responses=1" >> $sysctlc12

# block SYN-flood Attack
# Protects against DDoS attacks rich in TCP connections
echo "net.ipv4.tcp_syncookies = 1" >> $sysctlc12
echo "net.ipv4.tcp_max_syn_backlog = 4096" >> $sysctlc12
echo "net.ipv4.tcp_synack_retries = 3" >> $sysctlc12

# mitm route attack 
# Запрещает маршрутизацию через ваш сервер (source routing)
# Forbid routing throught your server (source routing)
echo "net.ipv4.conf.all.accept_source_route = 0" >> $sysctlc12
echo "net.ipv4.conf.default.accept_source_route = 0" >> $sysctlc12

# ipv6 disable
echo "# net.ipv6.conf.all.disable_ipv6 = 1" >> $sysctlc12
echo "# net.ipv6.conf.default.disable_ipv6 = 1" >> $sysctlc12
echo "# net.ipv6.conf.lo.disable_ipv6 = 1" >> $sysctlc12
#
...
sysctl -p
 ...
 [ -f $sysctl13 ] || touch $sysctl13 | printf "${C_WHITE} Настройка sysctl для ${vercheck}${C_RESET}\n"
 
# sysctl13="/usr/lib/sysctl.d/50-custom.conf"
tee $sysctl13 &>/dev/null << EOF
#
#       The small Sysctl debian tweaks
...
#ICMP ingore - option if you want
...
# increase of limits for hi-load servers
net.core.somaxconn=65535
net.core.netdev_max_backlog=65535
net.ipv4.ip_local_port_range=1024 65535

# Ускорение закрытия соединений
# fasters close of connection
net.ipv4.tcp_fin_timeout = 30
net.ipv4.tcp_tw_reuse = 1

# Защита от Small SYN flood
# Defence from a Small SYN flood
net.ipv4.tcp_mtu_probing = 1
EOF

#printf "${C_WHITE} Изменения systemctl $vercheck внесены ${C_RESET}"
sysctl --load $sysctl13
else
